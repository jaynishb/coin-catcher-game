...
const Row = ({ account, rank, highScore }) => {
  ...
  return (
    <tr
      className={`${
        currentAccount == account ? "bg-teal-500 px-2 text-white font-bold" : ""
      }`}
    >
...
export const Leaderboard = ({ start }) => {
  ...
  const getLeaderboard = () =>
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/leaderboard`)
      .then(({ data: { data: values } }) => setData(values));
  ...
  return (
    <>
      ...
      <h1 className="text-2xl p-2 mx-2 text-center	">Leaderboard</h1>
        <table className="items-center w-full border-collapse text-blueGray-700  ">
          ...
          <tbody>
            {data.length ? data?.map((row, index) => (
              <Row {...row} rank={index} />
            )) : <tr ><td className={'text-l p-4 text-center w-full'}>No one has played yet, connect and start apearing</td> </tr>}
          </tbody>
        </table>
      </div>
    </>
