...
export default () => {
  ...
  useEffect(() => {
    if (account) {
      axios
        .get(`${process.env.REACT_APP_BASE_URL}/add-account?account=${account}`)
        .then(console.log);
    }
  }, [account]);
  ...
  return (
    <div className="page">
      {!start && (
        <div className={`flex-1 flex items-center justify-center flex-col`}>
          <div className="sm:w-full md:w-1/3 m-2">
            ...
            <button
              className="my-2 w-full border-2 border-sky-500 hover:bg-sky-700 text-sky-500 hover:text-white-500 font-bold py-2 px-4 rounded"
              onClick={() => (active ? disconnect() : setIsOpen(true))}
            >
              {!active
                ? "Connect to wallet"
                : `Connected with ${truncateEthAddress(account)}`}
            </button>
            <button
              disabled={!active}
              className={`w-full bg-sky-500 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded ${
                !active ? "bg-gray-400  hover:bg-gray-700" : ""
              }`}
      ...
      <Confetti numberOfPieces={showConfetti} onConfettiComplete={confetti => {
          setShowConfetti(0)
          confetti.reset()
        }} recycle={false} width={window.width} height={window.height} />
      <SelectWalletModal isOpen={isOpen} onClose={() => setIsOpen(false)} />
      <div
        className={`${!start ? "hidden" : "visible"} w-full h-full`}
        ref={sceneContainer}
      >
        {size && size.width && size.height && start && (
          <Scene
            start={start}
            width={size.width}
            height={size.height}
            center={center}
            onGameStateChange={setStart}
          />
        )}
      </div>
    </div>
